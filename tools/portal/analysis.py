"""Analysis helpers for downloaded portal replays."""
...
def simulate_hand(
    ...
    for idx, entry in enumerate(entries):
        ...
        if len(remaining) <= 1:
            continue
        active = [s for s in remaining if s not in all_in]
...
def analyze_match_payload(
    payload: dict,
    stats: dict[str, Counter],
    match_results: dict[str, Counter],
) -> None:
    ...
    for hand in sorted(payload.get("hands") or [], key=lambda row: row.get("hand_num", 0)):
        simulate_hand(hand, seat_to_bot, stats)


def analyze_match(path: Path, stats: dict[str, Counter], match_results: dict[str, Counter]) -> None:
    analyze_match_payload(load_json(path), stats, match_results)
...
def build_report(directory: Path, top: int) -> dict:
    ...
    rows = [
        row_from_counts(bot_id, counts, bots, official, match_results)
