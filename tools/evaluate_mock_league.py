...
def sixmax_specs(pool: dict[str, str], table_size: int = 6, max_tables: int | None = None) -> list[dict]:
    items = list(pool.items())
    if len(items) < 2:
        raise ValueError("need at least two bots for a league")
    if len(items) <= table_size:
        return [{"name": "league_sixmax_1", "bots": dict(items)}]
    limit = min(len(items), max_tables or len(items))
    specs = []
    for start in range(limit):
        chunk = [items[(start + offset) % len(items)] for offset in range(table_size)]
        specs.append({"name": f"league_sixmax_{start + 1}", "bots": dict(chunk)})
    return specs


def heads_up_specs(pool: dict[str, str], max_pairs: int | None = None) -> list[dict]:
    items = list(pool.items())
    pairs = list(combinations(items, 2))
    if max_pairs is not None:
        pairs = pairs[:max_pairs]
    return [
        {"name": f"league_hu_{left[0]}_vs_{right[0]}", "bots": {left[0]: left[1], right[0]: right[1]}}
        for left, right in pairs
    ]


def build_specs(pool: dict[str, str], mode: str, table_size: int = 6, max_tables: int | None = None, max_pairs: int | None = None) -> list[dict]:
    if mode == "sixmax":
        return sixmax_specs(pool, table_size=table_size, max_tables=max_tables)
    if mode == "heads-up":
        return heads_up_specs(pool, max_pairs=max_pairs)
    specs = sixmax_specs(pool, table_size=table_size, max_tables=max_tables)
    specs.extend(heads_up_specs(pool, max_pairs=max_pairs))
    return specs
...
def summarize_bot_results(results: list[dict], bust_penalty: float = 9000.0, stdev_penalty: float = 0.15, error_penalty: float = 25000.0) -> list[dict]:
    ...
    return sorted(ranking, key=lambda item: (item["score"], item["mean_delta"]), reverse=True)
...
def run_league(args) -> dict:
    ...
    specs = build_specs(pool, args.mode, table_size=args.table_size, max_tables=args.max_tables, max_pairs=args.max_pairs)
