"""Coverage for the standalone rollout-search submission candidate."""
...
BOT_DIR = ROOT / "bots" / "rollout_search_submission"
BOT_FILE = BOT_DIR / "bot.py"
BEST_PARAMS = ROOT / "runs" / "rollout_search_tuning" / "rollout-final-20260531-004635" / "best_params.json"


def load_bot():
    spec = importlib.util.spec_from_file_location("rollout_search_submission_under_test", BOT_FILE)
...
def test_submission_file_is_single_file_and_static_safe():
    ...
    assert "tools.strong_mocks.policies" not in source
...
def test_embedded_params_match_tuned_rollout_data_when_present():
    bot = load_bot()
    if BEST_PARAMS.is_file():
        expected = json.loads(BEST_PARAMS.read_text(encoding="utf-8"))
    else:
        expected = {
            "samples": 1024.0,
            "deep_samples": 1024.0,
    ...
    assert set(bot.PARAMS) == set(expected)
    for key, value in expected.items():
        assert bot.PARAMS[key] == pytest.approx(float(value), abs=1e-12)


def test_fg_functions_have_expected_tuned_shape():
    bot = load_bot()

    assert bot._check_raise_fraction(0.20) == 0.0
    assert 0.0 < bot._check_raise_fraction(0.50) < bot._check_raise_fraction(0.85)

    assert bot._facing_bet_raise_fraction(0.20, 0.50, 2, 0.25) < 0.0
    assert bot._facing_bet_raise_fraction(0.53, 0.50, 2, 0.25) == 0.0
    assert bot._facing_bet_raise_fraction(0.97, 0.20, 2, 0.05) > 0.0
    assert bot._facing_bet_raise_fraction(0.97, 0.20, 2, 0.50) == 0.0


def test_rollout_equity_is_bounded_and_seed_deterministic(monkeypatch):
    bot = load_bot()
    monkeypatch.setattr(bot, "TIME_BUDGET_SECONDS", 100.0)
...
def test_time_guard_stops_after_minimum_completed_samples(monkeypatch):
    bot = load_bot()
    state = state_template()
    calls = {"evaluate": 0}
